...
PHP_ARG_WITH(websockets-php-src, the path to use for websocket extension includes,
[ --with-websockets-php-src The path to the php-src directory for extra websocket includes], no)
...
if test "$PHP_WEBSOCKETS" = "yes"; then
  ...
  if test "x$PHP_WEBSOCKETS_PHP_SRC" == "xno"; then
    PHP_WEBSOCKETS_PHP_SRC=/usr/include/php
  fi
  ...
  if test "x$PHP_WEBSOCKETS_HTTPD_INCLUDES" == "xno"; then
	...
	PHP_ADD_INCLUDE(${PHP_WEBSOCKETS_PHP_SRC}/sapi/apache2handler)
	PHP_ADD_INCLUDE(${PHP_WEBSOCKETS_PHP_SRC}/sapi/apache_hooks)
